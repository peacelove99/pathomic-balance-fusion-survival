...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        slide_data = pd.read_csv(csv_path, low_memory=False)  # 读取 CSV 文件
        # slide_data = slide_data.drop(['Unnamed: 0'], axis=1)
        if 'case_id' not in slide_data:  # 生成 case_id 列
        ...
        patients_df = slide_data.drop_duplicates(['case_id']).copy()  # 'case_id' 去重 -> patients_df 病人
        uncensored_df = patients_df[patients_df['censorship'] < 1]  # 'censorship'<1 -> uncensored_df 未截断病人

        disc_labels, q_bins = pd.qcut(uncensored_df[label_col], q=n_bins, retbins=True, labels=False)  # 未截断数据分到4个区间内
        q_bins[-1] = slide_data[label_col].max() + eps  # 稍微扩充区间边界
        q_bins[0] = slide_data[label_col].min() - eps  # 稍微扩充区间边界

        disc_labels, q_bins = pd.cut(patients_df[label_col], bins=q_bins, retbins=True, labels=False, right=False, include_lowest=True)  # 重新切割数据
        patients_df.insert(2, 'label', disc_labels.values.astype(int))  # 区间作为新的 label

        patient_dict = {}
        slide_data = slide_data.set_index('case_id')
        for patient in patients_df['case_id']:  # 生成患者字典
        ...
        self.label_dict = label_dict  # 更新标签
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str = 'train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(df_slice, metadata=self.metadata, mode=self.mode, signatures=self.signatures,
